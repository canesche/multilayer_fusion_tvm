...
def execute_autoTVM(tag_name, func, N, H, W, CO, CI, KH, KW, stride, padding, order, number):

    d_tvm = tvm.nd.array((np.random.uniform(size=(N, CI, H, W))).astype(dtype), device=dev)
    f_tvm = tvm.nd.array((np.random.uniform(size=(CO, CI, KH, KW))).astype(dtype), device=dev)
    b_tvm = tvm.nd.array((np.random.uniform(size=(1, W, 1))).astype(dtype), device=dev) 

    task = autotvm.task.create(tag_name, args=(N, H, W, CO, CI, KH, KW, stride, padding, order), target="llvm")
...
if __name__ == "__main__":
    ...
    stride = (1, 1)
    padding = (1, 1)
    #interval = [128, 256, 512, 1024, 2048, 4096]
    interval = [4096]
    ...
    for l, i in enumerate(interval):
        ...
        for j in range(len(order)):
            r.append(execute_autoTVM("fusion", fusion, N, H, W, CO, CI, KH, KW, stride, padding, order[j], j))
            if j > 0:    
